...
public class FileUploadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		// 파일의 경우 reqeust.getpart를 통해서 얻을 수 있다.
		logger.debug("contentType: " + request.getContentType());
		logger.debug("userId : " + request.getParameter("userId")); // fileUpload.jsp에서
																	// 갖가온 name값
		// logger.debug(request.getParameter("uploadFile"));
		...
		String contenDisposition = uploadFilePart
				.getHeader("Content-Disposition");
		logger.debug("contenDisposition: " + contenDisposition);

		String fileName = FileUtill.getFileName(contenDisposition);
		...
		// D:\A_TeachingMaterial\7.Jspspring\workspace\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\jsp\fielUpload
...
//			uploadFilePart.write(path + File.separator + fileName);
			...
			//사용자가 업로드한 실제 파일명은 디비상에 저장하고 , 물리적 파일명은 임의로 작용
